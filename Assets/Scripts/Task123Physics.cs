...
public class Task123Physics3D : MonoBehaviour
{
    ...
    private void UpdateTask1()
    {
        ...
        float distance = Mathf.Abs(speed1 * currentTime);
        ...
        if (outputDistance1 != null) outputDistance1.text = distance.ToString("F2");
    }

    private void UpdateTask2()
    {
        float x = startCoord2 + speed2 * currentTime;
        float distance = Mathf.Abs(speed2 * currentTime);
        ...
        if (outputDistance2 != null) outputDistance2.text = distance.ToString("F2");
    }

    private void UpdateTask3()
    {
        float v = startSpeed3 + acceleration3 * currentTime;
        float x = startCoord3 + startSpeed3 * currentTime + 0.5f * acceleration3 * currentTime * currentTime;

        accumulatedDistance3 += Mathf.Abs(v) * Time.deltaTime;
